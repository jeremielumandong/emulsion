...
Choose the rendering medium before choosing the nib. Follow explicit pencil, brush-ink, marker, watercolour, painterly or vector instructions and combine that medium's playbook with the construction guidance here. For an otherwise unspecified monochrome manga drawing, ink with reserved whites and selective tone is a reasonable stated assumption; it does not require every available effect.

Discover a working palette with list_brushes, then assign the needed roles from actual settings/swatches:
- Construction: when a separate sketch helps an ink-and-tone brief, Blue pencil or Sketch pencil for light gesture and volumes. For other media use compatible light marks; a pen-only sketch can construct with its chosen pen.
- Expressive contours and broad blacks: G-pen for nib accents, Fude brush for broad pressure-shaped marks; filled paths or fill_selection for deliberately flat closed masses. Fine facial strokes are not a practical way to build large dark shapes.
- Small features and even lines: Maru pen for fine tapered features; Kabura or Milli pens for quieter even lines where appropriate. Use fewer fine marks than structural contours.
- Values: Screentone 20%/40%/60% for printed dots, Hatching/Cross hatch for patterned tone, or Ink wash for a diluted-ink treatment. These are alternative mark languages, not a checklist. Distinguish the Hatching preset's grain from the hatch tool's generated individual strokes. Use the reference and requested finish to choose; flat grey fills are also valid.
- Colour: discover Marker, Watercolour or the requested paint family and use that medium's technique for masses and transitions. A coloured cover or watercolour manga request should not silently become pen outlines with flat grey patches.
- Cleanup and accents: a discovered eraser with mode=erase for removing raster mistakes; White ink only for intentional opaque white accents on the correct layer. Speed lines belong to action effects only when needed.

Inspect a candidate beyond the first catalog page when it fits a needed role. Preview a short feature and broad mark at intended settings when unsure of pressure/taper behaviour. A monochrome pen-only sketch is valid when that is the requested finish; otherwise select the mark needed by each passage rather than continuing with the first pen. Review the medium character as well as construction: do broad blacks, fine features and values read as the chosen techniques at delivery size?
...
- Establish the scalp envelope and hair part, then a few large connected locks and a clear black/white pattern. Vary their size and curvature unless repeated angular locks or evenly spaced strands are intentional in the requested design. Construct hands from palm, thumb and finger groups with a believable wrist and grip before nails or creases.

For action panels and interacting characters:
- Block both figures, their eye lines and the point of contact in one construction pass. Trace each participating shoulder–elbow–wrist chain; make the bend and foreshortening legible with overlaps and changing cross-sections. Compare the reach and hand scale across the scene before inking. Preserve deliberate exaggeration while keeping joint connections coherent.
- Construct the hand and held object together: establish the palm plane, thumb opposition, finger groups, object axis and which edges disappear behind the grip. Carry a rigid blade or shaft on a consistent axis and width progression through occlusion. Check the contact crop before effects; an impact burst should emphasize a readable interaction, not conceal an unresolved one.
- Judge the action without speed lines, bursts or dense hatching, keeping these on separate layers when used. Add them only after the pose and contact read. Aim directional effects at the intended action and leave quiet space around expressions and essential contours; vary their length and spacing rather than filling the perimeter mechanically.
- Check the face as a single turned volume before adding angular shadow patches or expression lines. Keep cheek, jaw, neck and facial feature planes consistent with that turn. Angular manga can be intentional; unrelated corners and shadow shapes that obscure form need correction.

For an ink-and-tone brief, organize editable work into the needed Construction, Tone, Blacks, Contours and Features nodes. Place each tone below its corresponding ink. Hide construction only after checking the finished shapes. Use draw_path for silhouettes, closed black masses, eyelids and hair locks that need editable geometry; use pressure-sensitive paint for brush character. A path stroke has constant width: use a closed filled wedge for an editable taper, or pressure-sensitive paint for expressive raster ink. Do not try to repair a raster stroke with set_path. Other media use their own layer roles; a no-ink watercolour needs washes and pigment, not obligatory black masses or pen contours.

Inking choices (only for passages that call for ink):
...
- Plan solid blacks and white reserves before tone. When starting without a user selection, use a temporary subject-shaped selection for a tone region: draw_path with stroke/fill "none" can define the boundary, then path_to_selection, paint, deselect before unrelated ink. Selections are snapshots; rebuild temporary ones after changing their source path. If the person already has a selection, preserve it and work within it under the studio selection rules.
...
Review checkpoints for ink-and-tone work: (1) gesture, head turn, feature placement and hand construction; (2) expression, lid/iris overlaps, hair masses and ink hierarchy; (3) confined tone, black/white balance and legibility at delivery size. For other media, keep these construction checks but use the selected medium's development and finish checks; do not add ink or tone to satisfy this list. Inspect both a face/hand crop and the full composition. Correct the most consequential visible mismatch before adding decoration, within the studio review budget. Do not automatically rebalance a centred portrait or remove intentional symmetry.
...
```json
[
  {"name":"list_brushes","arguments":{"query":"Manga"}},
  {"name":"list_brushes","arguments":{"query":"Screentone 20%"}},
  {"name":"list_brushes","arguments":{"query":"G-pen"}},
  ...
  {"name":"get_view","arguments":{"max_size":800}},
  {"name":"critique","arguments":{"context":{"medium":"pen-and-ink with screentone","style":"clean black-and-white manga character ink with restrained dot tone","stage":"final ink","composition_intent":"quiet three-quarter head study","user_constraints":["Preserve the head turn and coherent gaze","Check lid and iris overlaps before adding details","Keep neck tone confined to its shadow boundary","Preserve white space"]}}}
]
```
