...
mod tests {
    ...
    fn tool_returns_intent_and_composition_and_detail_images() {
        ...
        let context = json!({"medium": "manga", "stage": "gesture", "composition_intent": "centred symmetric portrait", "user_constraints": ["keep symmetry"]});
    ...
    fn mcp_json_rpc_carries_images_mapping_and_context() {
        ...
        let calls = [
            ...
            json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"critique","arguments":{"context":{"medium":"watercolour","composition_intent":"centred"}}}}),
        ...
        let mut out = Vec::new();
        crate::server::serve(
            Host(Editor::new(Document::new(100, 100), None)),
            input.as_bytes(),
            &mut out,
        )
        .unwrap();
        ...
        .unwrap();
        assert_eq!(review["context"]["composition_intent"], "centred");
    }
}
