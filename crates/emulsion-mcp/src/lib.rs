//! `emulsion-mcp` — Emulsion's MCP server, which exposes editor commands to a coding CLI.
//!
//! Phase 0: a newline-delimited JSON-RPC 2.0 server on stdio that answers
//! `initialize`, `tools/list`, `tools/call`, and `ping`, with an empty tool
//! set. Tools arrive with the Command API in Phase 2. stdout carries protocol
//! only; logs go to stderr via `tracing`.
...
/// Run the server over this process's stdin/stdout until EOF.
pub fn serve_stdio() -> anyhow::Result<()> {
    serve(EmptyHost, std::io::stdin().lock(), std::io::stdout().lock())
}
