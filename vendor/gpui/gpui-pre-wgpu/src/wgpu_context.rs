...
impl WgpuContext {
    ...
    async fn select_adapter_and_device(
        ...
        if force_software {
            log::info!("Software rendering explicitly requested");
        }
        adapters.retain(|adapter| {
            crate::adapter_policy::allowed(
        ...
        if adapters.is_empty() {
            anyhow::bail!(
                "No compatible graphics adapters found (forced software: {force_software}). \
                 For CPU rendering on Linux, install a Vulkan software driver such as Mesa Lavapipe."
            );
        }
        ...
        // 2. Hardware before software, including a software compositor's adapter.
        ...
        // 5. Backend — prefer Vulkan/Metal/Dx12 over GL/etc.
