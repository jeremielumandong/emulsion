...
fragment float4 polychrome_sprite_fragment(
  ...
  constexpr sampler atlas_texture_sampler(mag_filter::linear,
                                          min_filter::linear);
  // Atlas images have no padding. Keep linear filtering inside this image's
  // texel centers so neighboring atlas allocations cannot bleed into its edges.
  ...
  float2 first_texel = (tile_origin + 0.5) / atlas_size;
  float2 last_texel = (tile_origin + tile_size - 0.5) / atlas_size;
  float4 sample =
      atlas_texture.sample(atlas_texture_sampler,
                           clamp(input.tile_position, first_texel, last_texel));
  float distance =
