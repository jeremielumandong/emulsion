// Modified by Emulsion: economical Windows software-renderer frame scheduling.
use std::{
    sync::LazyLock,
...
impl VSyncProvider {
    ...
    pub(crate) fn wait_for_vsync(&self) {
        if let Some(interval) =
            crate::rendering_policy::frame_interval(crate::rendering_policy::software_rendering())
        {
            // Adapted from AgentOps' Apache-2.0 GPUI patch. Sleep on this
            // scheduler thread, never on the UI/input thread.
