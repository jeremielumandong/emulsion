...
fn get_adapter(
    ...
    let force_software =
        std::env::var("GPUI_FORCE_SOFTWARE_RENDERING").is_ok_and(|value| value == "1");
    let mut adapter_index = 0;
    let adapters =
        std::iter::from_fn(
            || match unsafe { dxgi_factory.EnumAdapters1(adapter_index) } {
                Ok(adapter) => {
                    adapter_index += 1;
                    Some(adapter)
                }
                Err(error) => {
                    if error.code() != DXGI_ERROR_NOT_FOUND {
                        log::warn!("Graphics enumeration failed; trying WARP: {error:#}");
                    }
                    None
                }
            },
        );
    crate::rendering_policy::select_adapter(
        force_software,
        adapters,
        |adapter| {
            let desc = match unsafe { adapter.GetDesc1() } {
                Ok(desc) => desc,
                Err(error) => {
                    log::warn!("Cannot read graphics adapter; skipping it: {error:#}");
                    return None;
                }
            };
            let name = String::from_utf16_lossy(&desc.Description);
            if crate::rendering_policy::is_software_adapter(
                (desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE.0 as u32) != 0,
                name.trim_matches('\0'),
            ) {
                return None;
            }
            match create_adapter_device(&adapter, debug_layer_available) {
                Ok((device, context, level)) => {
                    crate::rendering_policy::set_software_rendering(false);
                    log::info!("Using hardware renderer: {}", name.trim_matches('\0'));
                    Some((adapter, device, context, level))
                }
                Err(error) => {
                    log::warn!("Adapter {} unavailable: {error:#}", name.trim_matches('\0'));
                    None
                }
            }
        },
        || {
            // WARP also works when display-adapter enumeration has no usable
            // device. Preserve upstream capability checks and recovery.
            let adapter: IDXGIAdapter1 = unsafe { dxgi_factory.EnumWarpAdapter() }
                .context("No compatible hardware adapter; obtaining Windows WARP renderer")?;
            let (device, context, level) = create_adapter_device(&adapter, debug_layer_available)
                .context("Creating Windows WARP software renderer")?;
            crate::rendering_policy::set_software_rendering(true);
            log::info!("Using Windows WARP software renderer (30 Hz scheduler)");
            Ok((adapter, device, context, level))
        },
    )
}

fn create_adapter_device(
    ...
    Ok((
        device,
        context.context("D3D11 returned no device context")?,
        feature_level,
    ))
}
