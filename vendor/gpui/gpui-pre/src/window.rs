// Modified by Emulsion: avoid unchanged-frame keepalive on software renderers.
#[path = "presentation_policy.rs"]
...
impl Window {
    pub(crate) fn new(
        ...
        let mut software_renderer = platform_window
            .gpu_specs()
        ...
        platform_window.on_request_frame(Box::new({
            ...
            move |request_frame_options| {
                ...
                if force_render {
                    // Recovery can switch between hardware and software devices.
                    // Query only on forced redraws, not on every pointer frame.
                    if let Ok(Some(specs)) =
                        handle.update(&mut cx, |_, window, _| window.gpu_specs())
                    {
                        software_renderer = specs.is_software_emulated;
                    }
                }
                ...
                let needs_present = presentation_policy::needs_presentation(
